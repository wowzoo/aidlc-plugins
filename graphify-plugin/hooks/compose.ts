...
const PLUGIN_KEY = (() => {
  ...
  // LOCAL DELTA: `.aidlc-plugin` first — our plugins keep the manifest there, and without it
  // this loop always falls through to the parent-dir segment, which is the temp copy root
  // (`/tmp/pr-<n>` → "tmp") for every plugin. That collapses the per-plugin drops file the
  // comment above promises: a clean second compose then DELETES the first plugin's live
  // degraded drop, which is the exact failure this naming exists to prevent.
  for (const md of [".aidlc-plugin", ".claude-plugin", ".codex-plugin", ".kiro-plugin"]) {
    try {
...
// live degraded drop, so doctor went green. Per-plugin files isolate
...
try {
  ...
  for (const phase of existsSync(contribRoot) ? readdirSync(contribRoot) : []) {
    ...
    for (const file of files) {
      ...
      // silently misorder splices. Reject it up front.
      ...
      const requiredSections = (() => {
        ...
        for (const x of s.matchAll(/^    - (.+?)\s*$/gm)) {
          ...
          // stage with no signal — drop-log it instead.
      ...
      const blocksByAnchor = new Map<string, string[]>();
      {
        ...
        for (const line of body.split("\n")) {
          ...
          // fragment format with a nested fence corrupted the block.
